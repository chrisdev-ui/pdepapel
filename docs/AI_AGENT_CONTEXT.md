...
- **Homepage rebuild (2026-09-09, committed locally, NOT yet deployed).** Three additive migrations are written and validated on the local Docker database but **not applied to Railway**: `20260909_add_home_content.sql` (creates `HomeContent` + `HomeContentProduct` and copies existing slides/main banner), `20260909_add_product_available_at.sql` (`Product.availableAt`) and `20260909_add_newsletter_welcome_coupon.sql` (`NewsletterSubscriber.welcomeCouponId`, `interestProductId`). Apply the three before the deploy that ships commits `a4eb47e…2a9c680`; apply `20260909_drop_legacy_home_banners.sql` only after that deploy is live and the homepage was checked, because the previous code still reads `Billboard`/`MainBanner`/`Banner`. Optional admin env `NEWSLETTER_EARLY_ACCESS_SECRET` (≥16 chars) enables the early-access campaign email.
...
- `20260909_add_home_content.sql` — creates `HomeContent` (hero and campaign banner with schedule, campaign type, copy, image, `earlyAccessSentAt`/`arrivalSentAt`) and `HomeContentProduct`, then copies `Billboard` rows as HERO entries (only the newest active) and `MainBanner` as an inactive SEASON campaign; **not yet applied**. Apply before deploying the homepage rebuild.
- `20260909_drop_legacy_home_banners.sql` — drops `Banner`, `MainBanner`, `Billboard`; **apply only after** the homepage rebuild is live (the old code reads them).
- `20260909_add_product_available_at.sql` — additive nullable `Product.availableAt` + `[storeId, availableAt]` index («Próximamente»); **not yet applied**.
- `20260909_add_newsletter_welcome_coupon.sql` — additive `NewsletterSubscriber.welcomeCouponId` (unique) and `interestProductId` (+ index); **not yet applied**.
