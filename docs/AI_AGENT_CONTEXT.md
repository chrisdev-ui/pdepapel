...
- The image-analysis route accepts only HTTPS Cloudinary catalog URLs, at most three images per request, and uses Upstash Redis to enforce 20 new model requests per store per Colombia calendar day. A 24-hour cache is keyed by the exact public photos, selected category, and current color/design taxonomy; a cache hit is re-sanitized against the current taxonomy and does not consume another daily analysis. It returns only name/brand/color/design suggestions supported by the supplied photos. A deterministic color/design matching the store taxonomy is marked `existing`; a visible, reusable value not found in the taxonomy is marked `new` and stays a proposal. The administrator may explicitly confirm **Crear y usar** in the individual product form; only then does the normal authenticated Color/Design API create it and select it. Product-group forms never auto-create variant attributes from a group photo. No model output may create a SKU, GTIN, price, stock change, slug change, or automatic Mercado Libre update.
