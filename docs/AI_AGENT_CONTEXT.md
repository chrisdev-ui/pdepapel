...
- **Mercado Libre listings-management audit (2026-09-11, local commits, not yet deployed).** Second Publicaciones audit (managing and importing existing listings). Correctness: edit path maps nested color/size for prefill; bulk publish limited to DRAFT/ERROR without an open review rejection and never a listing being sent (`isMarketplaceListingPublicationInProgress`, shared with the publish route and DELETE); pause/activate only on ACTIVE/PAUSED; PATCH clears `publicationError` when the rejected step is edited, queues SYNC_LISTING_CONTENT for content edits and SYNC_STOCK for a buffer change on published listings, and gates on the connection/QStash like its siblings; content-sync failures write `lastError`; status sync maps the returned status through `getMarketplaceListingStatusFromRemote`; attribute text keeps `value_id`. Import: preview flags drafts, non-COP, catalog and status notes; `replaceDraft` required to overwrite a draft; item-scoped re-read with bounded concurrency and partial results; shared status mapper; named errors; `metadata.currencyId/catalogListing`. Metadata: builder merges and stamps `version`; `updateMarketplaceListingMetadataGuarded` answers 409 on concurrent edits (no column added). Sale-conditions GET is read-only. Client errors are `MercadoLibreRequestError` (AppError) with upstream details. UI: `listings/listing-table.tsx` on DataTable (`Models.MarketplaceListings`), row signals, side sheet, per-row pause/activate through the bulk route. No schema change. See §12 «Managing existing listings».
- **Public API cost-field exposure audit (2026-09-11, local commits, not yet deployed).** Unauthenticated storefront routes returned full Prisma rows: `GET /products` and `/products/[id]` (acquisition cost, transportation cost, supplier id and, on demand, the supplier), `GET /product-groups` (nested full products behind a dead `auth()`), `GET /suppliers`, `GET /products/catalog`, `GET /orders/[orderId]` (customer PII, order `token`, internal notes and profit fields, unscoped by store), `GET /coupons/[id]` (every order of the coupon), `GET …/reviews/[reviewId]` (moderation note) and every public review (reviewer Clerk id). Fixed through explicit selects, owner branches and owner-only guards; see §5 «Public routes return data only through the public selects». No schema change.
- **Mercado Libre publication wizard audit (2026-09-11, local commits ccbfed5…0a02405, not yet deployed).** `pdepapel-admin/prisma/manual-migrations/20260911_add_product_transportation_cost.sql` (nullable `Product.transportationCost DOUBLE DEFAULT 0`) and `20260911_backfill_product_transportation_cost.sql` (`UPDATE … SET transportationCost = 1000 WHERE 0 OR NULL`, 1980 rows) were applied to Railway on 2026-09-11 and verified through `information_schema` and a count query (1980 rows at 1000, none at 0). Same ordering rule as every `Product` column: the migration went first because the product form, the product search and the Mercado Libre selects read the column. The behaviour changes are documented in §12 «Publication guards and wizard resilience».
...
- `pdepapel-admin/prisma/manual-migrations/20260911_add_order_inventory_issues_and_ops_signals.sql` and `20260911_add_shipping_guide_attempt.sql` were applied to Railway on 2026-09-10 with `prisma db execute` right before deploying the matching code, and verified through `information_schema` (three new tables with their indexes, two nullable `Shipping` columns; 702 orders and 592 shipments untouched). The first adds three additive tables: `OrderInventoryIssue` (a stock line that `createInventoryMovementBatchResilient` could not move when an order was paid, cancelled or deleted — every call site now records it inside the same transaction instead of only logging; `orderId` is nullable so the debt outlives a deleted order; retried or reconciled from the order's care zone or from Movimientos via `POST /api/[storeId]/orders/inventory-issues/[issueId]`), `JobRun` (last run per cron / storefront revalidation, shown in Inicio's «Sistemas» card) and `ShippingWebhookEvent` (EnvioClick payloads that matched no shipment). The second adds `Shipping.guideError` / `guideAttemptedAt`, written when the automatic guide fails on «Marcar como pagado». Apply both before deploying the matching code: Inicio and the order pages query the new tables on every load. Fairs and point of sale deliberately do not record inventory issues yet (their own audit is pending).
- `pdepapel-admin/prisma/manual-migrations/20260910_add_store_low_stock_threshold.sql` was applied to Railway on 2026-09-10 with `prisma db execute`, immediately **before** pushing the product-surface rework, and verified through `information_schema` (nullable `int`, no existing row changed, 685 paid-or-sent orders untouched). The ordering is not optional: `checkIfStoreOwner` does an unscoped `store.findFirst`, so Prisma names every scalar column and `verifyStoreOwner` — which every admin mutation calls — would have failed on an unknown column, taking the panel and the storefront's API with it. Any future additive `Store` column has the same constraint: apply first, deploy second.
...
- Cloudinary for images/media.
  **Promotions, boxes and social posts (audit Grupo A, 2026-09-11).** One clock: a coupon or offer window is a range of Colombian calendar days stored as real UTC instants by `lib/promotion-window.ts` (00:00 of the first day to 23:59:59.999 of the last); forms send `yyyy-MM-dd`; every comparison uses `new Date()` and never `getColombiaDate()` (a shifted instant, only for formatting a wall clock); rows stored before the fix are realigned once by `prisma/manual-migrations/20260911_realign_promotion_windows.sql` under the migration protocol. States: `getPromotionStatus` reads `vencida` (dates win), `agotada` (coupon `usedCount >= maxUses`), `desactivada` (manual switch), `programada`, `vigente`; the daily crons and «Recalcular vigencias» only switch expired or exhausted promotions **off**, nothing re-enables one turned off by hand, and coupon `PUT` (Desactivar) keeps the window and counters. Coupon limits: `lib/coupon-availability.ts` counts unpaid `CREATED`/`PENDING` orders as reservations in validate, checkout, admin orders and order edits so several checkouts cannot share the last use (`usedCount` still increments on payment); `POST /coupons/validate` returns only `PUBLIC_COUPON_SELECT`; `getCouponDetail` returns usage, reservations and the last orders without personal data; batches go through `POST /coupons/batch`; deleting a coupon with orders is refused everywhere and bulk deactivate only flips `isActive`. Offers: input parsed by `lib/offers.ts` (at least one target, targets in the store, fixed amounts below every affected price); mutations and the cron call `invalidateStorePromotionsCache` (Redis + storefront revalidation); the engine never shows the internal `name` as a label (no label = no badge) and breaks ties by `createdAt`. Every write on coupons, offers, boxes and posts filters by `storeId`. Boxes: `lib/boxes.ts` validates 1–300 cm with one decimal (comma or dot), `DELETE /boxes/[id]` refuses a box referenced by shipments (`relationMode = "prisma"` would otherwise SetNull silently), `MeasurementInput` is a text input with a decimal draft, and `Box` has no weight (the calculator takes weight from products). Social posts: `lib/social-posts.ts` validates identifiers per network (bare id or pasted link), every post mutation revalidates `/nosotros`, and X (Twitter) is not selectable. Follow-ups (same day): `Shipping.box` is `onDelete: Restrict` (emulated by the Prisma client) and `Post` has `@@unique([storeId, social, postId])` (`20260911_add_post_unique_identifier.sql`); forms share `hooks/use-unsaved-changes-guard.ts` (`beforeunload` while dirty plus `confirmLeave()` on the back button; wired in the coupon, offer, box and post forms); the About page loads social embeds only after the visitor presses «Cargar publicaciones» (`lib/social-embeds-consent.ts`, remembered per browser, independent of the analytics consent) and offers direct links meanwhile; the Descuento column sorts by type then amount.
...
7. **Admin API authorization (audited 2026-09-10).** The admin middleware marks every `/api/*` path public, so each dashboard handler must do two things, in this order and before reading the body: `auth()` → 401 when there is no session, then `checkIfStoreOwner(userId, params.storeId)` (or `verifyStoreOwner`, which throws the 403 `AppError`) → 403 for a signed-in customer of another store. The audit found 20 handler files that only checked «signed in» (custom orders, quotations, invoices GET, WhatsApp, shipments, shipment cache/cancel, DANE cache, image cleanup, selectable products, order shipping) and `customers/search` with no authentication at all; all now carry the owner check. `tests/integration/api-authorization.test.ts` loads every handler in its table with a stranger's session and an anonymous request and asserts 403/401: **add every new dashboard-only handler to that table**. Public by design and therefore not in the table: storefront reads (`products` GET, `products/[id]` GET, `search/products`, `reviews` GET, `products/[id]/reviews` GET, `categories`, `catalog-options`, `public/*`, `google-merchant/feed`), customer-scoped bearer endpoints (`account/*`, `orders/[id]/account`, reviews POST/PATCH by the review's author, `coupons/validate`), checkout and gateway flows (`checkout`, `checkout/[orderId]`, `bold/checkout/[orderId]`, `shipment/quote`, `shipment/track`), newsletter confirm/unsubscribe/subscribe, schedulers and cron behind `SCHEDULER_SECRET`/`CRON_SECRET`, QStash-signed `internal/*`, and the provider webhooks. **Public routes return data only through the public selects (audited 2026-09-11).** Every unauthenticated GET that returns a model must use an explicit `select` from `lib/public-catalog.ts` (`PUBLIC_PRODUCT_SELECT`, `PUBLIC_PRODUCT_DETAIL_SELECT`, image/category/group/color/size/design selects) or `lib/public-orders.ts` (`CUSTOMER_ORDER_SELECT` + `toCustomerOrderResponse`, `PUBLIC_QUOTATION_SELECT`), never `include` or a bare `findMany` on `Product`, `Order`, `Review` or `Supplier`: an `include` returns every scalar, and that is how `acqPrice`, `transportationCost`, `supplierId`, `abcClassification`, `soldCount`, the order `token`, `adminNotes`, `internalNotes`, `documentId` and the profit fields were public until that date. A column added to `Product` or `Order` stays hidden until someone adds it to a public select on purpose. `GET /products/[id]` and `GET /orders/[orderId]` branch on `checkIfStoreOwner`: the owner gets the full row (the admin product picker, the Mercado Libre wizard and the restock form read costs from the detail route; the payment watcher polls the order route), everyone else the public select. `GET /orders/[orderId]` stays reachable by order id for the storefront's order page but is scoped by `storeId` and replaces `createdBy` with a `createdByAdmin` boolean; the two checkout POSTs and the public quotation route scope by store inside the query. `includeSupplier` no longer exists. `PUBLIC_REVIEW_SELECT` carries no `userId`; the storefront review form asks `GET /products/[id]/reviews/mine` with the session token. Dashboard-only reads that were public are now owner-only and in the authorization table: `product-groups` (+`/[id]`), `suppliers` (+`/[id]`), `products/catalog` (the PDF catalog of Productos, not a storefront route), `offers` (+`/[id]`), `boxes` (+`/[id]`, store-scoped), `coupons` GET and `coupons/[id]` (no longer includes the coupon's orders), `products/[id]/reviews/[reviewId]`. The product-list Redis key carries `PUBLIC_PRODUCTS_CACHE_VERSION`: bump it whenever the public shape changes so stale cached rows are never served (old keys expire within 15 minutes). Regression tests: `tests/unit/routes/products.test.ts`, `product-detail-cors.test.ts`, `product-review-mine.test.ts`, `tests/integration/public-api-exposure.test.ts`, and the authorization table. Still on the roadmap from that audit, not done: the internal `Offer.name` in the (now owner-only) offers payload, the ownership gap on `POST /shipment/create-guide` (any signed-in Clerk user), and `?skipCache=true` letting anyone bypass the product-list cache. Open item from the audit: the `dian`, `envioclick` and `mercadolibre` webhooks do not verify a provider signature (Bold and Wompi do); Mercado Libre re-fetches the resource before acting, the other two should be reviewed against what each provider offers. The store's Clerk redirect props use the Core 2 names (`forceRedirectUrl`, `signInFallbackRedirectUrl`); the four `NEXT_PUBLIC_CLERK_*_URL` env vars are optional and unused. **pdepapel-admin moved to `@clerk/nextjs` v6 on 2026-09-10 as well**: server code imports `auth`, `currentUser` and `clerkClient` from `@clerk/nextjs/server`, `auth()` is awaited everywhere, `clerkClient` is a function (`await (await clerkClient()).users…`, `getUserList()` returns `{ data, totalCount }`), and `lib/utils.ts` must stay free of server-only imports because client components import it. The admin `middleware.ts` keeps `/api(.*)`, `/iniciar-sesion(.*)` and the auth pages public and, for everything else, redirects browser navigations without a session to `/iniciar-sesion` (`redirectToSignIn`) and answers 401 to scripted requests — deliberately **not** `auth.protect()`, whose non-browser branch rewrites to `/clerk_<id>`, a path the `[storeId]` segment would render. Unit tests mock `@clerk/nextjs/server` (with `clerkClient: async () => ({ users: … })`). **Panel access is not the same as a session** (`lib/admin-access.ts`): the storefront and the panel share one Clerk instance, so every shop customer can sign in on the panel's domain. `hasAdminAccess(userId)` is true only for users who own a store or who are listed in the optional `ADMIN_ALLOWED_USER_IDS` env var (comma-separated Clerk user ids, needed once to create the first store). The root layout sends any other session to `/sin-acceso` (public route with a sign-out button) instead of the store creator, and `POST /api/stores` answers 403 for them. The panel has **no sign-up page**: `/crear-cuenta` and `/sign-up` redirect to `/iniciar-sesion`, which is a branded full page (`components/auth/auth-page-shell.tsx`, `lib/clerk-appearance.ts`, Clerk's own header hidden). Dev-only footgun kept as is: `(dashboard)/[storeId]/layout.tsx` skips the owner filter when `NODE_ENV === "development"`.
...
- **Shared sell panel (2026-09).** `components/sales/sell-panel.tsx` is the one in-person sale screen (scanner, manual code, picker slot, cart, cash or transfer, confirm dialog); cart rules live in `lib/sell-cart.ts` (product lines capped by stock or fair reserve, capsules as fixed single lines keyed by QR, pure and unit-tested). A `SellSource` adapter supplies `lookup`, `submit` and the picker: the point of sale (`ventas-rapidas/components/sell-panel.tsx`) uses the catalog and `/api/[storeId]/point-of-sale/*`; the fair workspace uses only reserved items and capsule QRs through `/api/[storeId]/fair-events/[id]/{lookup,sales}` (a `Combobox` of reserved items as picker). Both surfaces render the same two `SectionCard`s («Productos», «Cobro»); cart lines carry `kind: "product" | "capsule"` and an explicit `capsuleCode` (`productLine`/`capsuleLine`/`toSaleItems` in `lib/sell-cart.ts`), and `lockedReason` disables the whole panel with a status callout (used while a fair is reconciling). `TintBadge` lives in `components/ui/tint-badge.tsx` (re-exported from `pedidos/components/order-badges.tsx`). Inventory rules are unchanged: the server applies stock once per sale, fairs sell only reserved units, capsules keep their real product. The fair e2e spec now confirms through the shared dialog («Registrar pago» → «Sí, registrar pago»).
...
- **Admin Ferias (2026-09, fourteenth redesign subtask).** `(routes)/ferias` lists fairs as cards with views in the URL (`vista`: `activas`, `cerradas`, `todas`), a sold-share bar, capsule count and a next-step button; "Nueva feria" is a dialog. `lib/fair-phases.ts` maps `FairEventStatus` to the phases Preparar › Vender › Conciliar › Cerrada, the status badges and the next step. The fair page keeps `fair-event-workspace.tsx` (reserve, capsules, sell, reconcile; closing is irreversible) under `fair-phase-header.tsx` with the phase stepper and anchors `#inventario #capsulas #ventas #cierre`. The sell screen inside a fair IS the shared `SellPanel` (see "Shared sell panel" above): fair sales go through `/api/[storeId]/fair-events/[id]/{lookup,sales}` with reserved stock and capsule QRs.
- **Admin Mercado Libre (2026-09, thirteenth redesign subtask).** `(routes)/mercadolibre` is one page with six tabs in the URL (`tab`: `resumen`, `publicaciones`, `ventas`, `preguntas`, `envios`, `anuncios`; `?listing=` opens Publicaciones and `?order=` opens Ventas). Resumen keeps the seller-account and secure-processing cards (they move to Ajustes › Integraciones when that subtask lands) plus the health cards and cash-flow summary; `components/operations-center.tsx` took a `sections` prop so the same data loader renders only the requested blocks (resumen, preguntas, reclamos, envios, rentabilidad) per tab. Ventas = the sales list (`historical-sales.tsx` composition over `DataTable`, views `?vista=` por-atender|pagadas|devueltas|todas from `lib/mercadolibre/sales-views.ts`, row actions resync / confirm return, the import card for pre-integration sales) + profitability; the operations center fetches only the endpoints its visible sections need. Anuncios = Product Ads, with videos still uploaded from each publication. Business rules are untouched: decoupled marketplace price, stock minus safety buffer, net collected or «Liquidación pendiente», human confirmation for listing links.
...
- `20260910_add_store_low_stock_threshold.sql` — additive nullable `Store.lowStockThreshold` (units); applied to Railway on 2026-09-10 with `prisma db execute` and verified through `information_schema`. `NULL` means "use the application default", `TRESHOLD_LOW_STOCK = 5`, resolved by `lib/product-readiness.ts › resolveLowStockThreshold`, so old rows need no backfill and every reader fails safe. Editable in Configuración; it drives the «Stock crítico» view and badge on Productos and Inventario.
...
### Taxonomy: categories, subcategories, sizes, colors, designs (audit Grupo B, 2026-09-11)

- **Vocabulary:** Prisma `Type` is the UI «Categoría» (top level, has an icon) and Prisma `Category` is the UI «Subcategoría» (has the storefront page, cover and SEO). Copy says "categoría"/"subcategoría"; never "tipo" or "Sub-Categoría". The hub is `/[storeId]/atributos?tab=categorias|subcategorias|tamanos|colores|disenos|opciones` (`&vista=archivados` for archived rows); the old list pages `tipos`, `categorias`, `tamanos`, `colores`, `disenos` redirect there preserving `vista`, and every form returns to its hub tab. The hub reads tab and view from the URL on every render (sidebar clicks work while already on the page), shows a create link on each empty state, and counts only active subcategories under catalog options.
- **API rules (`lib/taxonomy.ts`):** every read or write by id is scoped to `{ id, storeId }`, and detail loaders (`getType/getSize/getColor/getDesign(storeId, id)`, `getCategoryTypes(storeId)`) return `null` for another store's row. Names are unique without regard to case or accents: colors, designs and size names per store, subcategories per store + category; `findDuplicateTaxonomyName` checks before the write and the `@@unique([storeId, name])` indexes on `Color` and `Design` (migration `20260911_grupo_b_suppliers_and_taxonomy.sql`, applied to Railway on 2026-09-11 after merging the production duplicates: Rojo, Degrade, Osito, Imperio papelero) catch the race; both surface the same Spanish 409. Every successful create, rename, delete or archive calls `invalidateStoreProductsCache(storeId)`; subcategories also revalidate their `/categoria/<slug>` paths. Creating a subcategory never calls OpenAI: covers and intros are generated only through `POST /api/[storeId]/categories/[categoryId]/cover` with `{ part: "cover" | "intro" | "both", force }`, which persists the result. Deletes stay refused while products use the item; the type form and the hub refuse to delete a category with subcategories and explain the cascade, and «Archivar» is the preferred care action (`AttributeCareCard`, `POST /attributes/archive`).
- **Icons:** a `Type` carries its icon in two exclusive fields. `icon` is a Lucide name in kebab-case (validated only by the pattern `^[a-z0-9-]{2,32}$`); `iconSvg` is the sanitised inner markup of an AI-generated 24×24 Lucide-style icon. Both apps resolve `iconSvg` → `icon` → keyword fallback from slug/name (`lib/taxonomy-icons.ts` in the admin, `lib/type-icons.tsx` in the store) → `tag`. Every write to `iconSvg` passes through `lib/svg-icon.ts` (identical copy in the store): only `path|circle|line|rect|polyline|polygon|ellipse` with geometry attributes survive, presentation attributes are stripped, anything else is a 400, and renderers build React elements and add the Lucide wrapper attributes themselves, never `dangerouslySetInnerHTML`. Type names never contain emoji: POST/PATCH strip a leading symbol (`stripTaxonomyIcon`) and the migration cleaned the 14 production rows, mapping each to the Lucide name the store already chose by keyword. The picker (`components/ui/icon-picker.tsx`) shows the 16 curated icons as tiles, searches all Lucide icons visually with Spanish synonyms, and loads the icon index lazily; `POST /api/[storeId]/types/icon-suggestions` (owner-only, 20 requests per 10 minutes per store, 503 without `GEMINI_API_KEY`) returns up to three sanitised proposals and never assigns one by itself. The store renders arbitrary Lucide names through a lazy `DynamicIcon` (`lib/type-icon-dynamic.tsx`) so the icon map stays out of the initial bundle.
- **Form pattern:** the five taxonomy forms, the supplier form and the restock draft form share the section-card chrome (`SectionCard`, `FormPageHeader`, sticky footer, usage aside, `useUnsavedChangesGuard`, `useFormPersist` for new records only, Spanish Zod messages, single column on phones). The subcategory slug is not editable from the form: the API regenerates it from the name and keeps the previous slug as a `CategorySlugAlias`, so the form shows the URL read-only and says so.
...
- **Vercel Firewall on `pdepapel-store` must not challenge `/api/revalidate`.** The admin calls it server to server (axios from a Vercel function); the WAF managed rule **Bot Protection** set to «Challenge» (and Attack Challenge Mode) answers `429` with `x-vercel-mitigated: challenge` before the route runs, so the store's function logs show nothing and the catalog stays stale. On 2026-09-11 Bot Protection was switched to challenge at 01:23 and the first «Marcar como pagado» of the day failed to revalidate. Keep a custom rule with action **Bypass** for path `/api/revalidate` (Vercel › pdepapel-store › Firewall › Rules), or set Bot Protection to «Log»; the same applies to any other non-browser client of the store (Playwright in `public-health.yml`, uptime monitors). `lib/revalidate-store.ts` names the mitigation in the alert (`describeRevalidationFailure`), retries once on network errors and 5xx, and never retries a challenge.
...
- **Order status and shipment status tell one story (2026-09-11).** `reconcileShipmentStatus` (`lib/order-transitions.ts`) runs inside `PATCH /api/[storeId]/orders/[orderId]`: «Marcar como enviado» leaves `Shipping.status` on `Shipped` when it was still `Preparing` (also when the request carries no `shipping` block, via a `shipping.update` in the same transaction), and saving the form with the shipment set to `Shipped`/`PickedUp`/`InTransit`/`OutForDelivery`/`Delivered` moves a `PAID` order (or a cash-on-delivery `PENDING` one) to `SENT`. It never forces a transition `canTransition` forbids and never touches inventory (`PAID` and `SENT` are both paid-like). A manual delivery (`ShippingProvider.MANUAL`, e.g. a domiciliario) can be marked as sent without a guide number; only EnvioClick shipments without a guide still require one. Before this, an order could read «Enviado» with the shipping badge still on «Preparando», and admins typed fake guide numbers to get past the dialog.
- **The order page's «Siguiente paso» buttons act, they do not only scroll.** `getNextStepCard` (`lib/order-timeline.ts`) may attach `action: "pay" | "ship"` to a link; `components/next-step-panel.tsx` scrolls to the anchor and dispatches the `pdepapel:order-action` DOM event (`lib/order-actions.ts`), and the `StatusActions` card variant inside the Pago card opens the matching confirmation dialog. Every `#anchor` in a next-step card must exist on the page (`#pago`, `#envio`, `#cliente`, `#descuentos`, `#zona-de-cuidado`); `#estado` never existed and the header's «Marcar como pagado» did nothing until 2026-09-11.

### Supply chain: suppliers and restock orders (audit Grupo B, 2026-09-11)

- **Restock orders (`RestockOrder`, UI «Aprovisionamiento») follow a fixed lifecycle** in `lib/restock-orders.ts`: `DRAFT → ORDERED` (manual, «Pedir al proveedor»), `ORDERED/PARTIALLY_RECEIVED → COMPLETED` (by receipt when every line is covered, or manually to close short), `DRAFT/ORDERED → CANCELLED` only while `receivedUnits === 0`, `CANCELLED → DRAFT` to retake. `PARTIALLY_RECEIVED` is never chosen by hand. Lines, supplier and shipping cost are frozen once ordered (`areRestockLinesLocked`); notes are saved on their own through `PATCH { notes }`. Only drafts and cancelled orders without receipts can be deleted; everything else is purchase history.
- **Order numbers come from the highest existing number plus one** (`nextRestockOrderNumber`, `allocateRestockOrderNumber` inside the creating transaction, retried on `P2002` by `withOrderNumberRetry`), shared by the API and the products batch import. Never derive a number from `count + 1`: deleting a draft used to lower the count and reuse a number.
- **Receiving cannot double stock.** `POST /api/[storeId]/restock-orders/[orderId]/receive` takes `{ idempotencyKey, updateCosts, assignSupplier, lines: [{ restockOrderItemId, quantity, allowExcess }] }`. The dialog generates the key when it opens; the route creates a `RestockOrderReceipt` (`@@unique([restockOrderId, idempotencyKey])`, migration `20260911_grupo_b_suppliers_and_taxonomy.sql`) before touching inventory, so a replay is a 409 and stock stays put. Each line is capped at what is still owed unless `allowExcess` is set for that line (the dialog asks for an explicit tick and the receipt records `excessUnits`); receipts are refused on `DRAFT`, `COMPLETED` and `CANCELLED`; the status check and the plan run on a fresh read inside the transaction. Movements are `RESTOCK_RECEIVED` with `referenceId = order.id`, `description = "Recepción <receiptId>"` and `cost = landed unit cost` (`landedUnitCost`: unit cost × (1 + shipping / merchandise total), factor 1 when either is zero). With `updateCosts` (default on) the product's `acqPrice` becomes the unit cost of this purchase and `transportationCost` the per-unit shipping share, so margins and the Mercado Libre price floor follow real purchases; `assignSupplier` fills `Product.supplierId` only when it is empty. The route then calls `invalidateStoreProductsCache`.
- **UI:** the list (`aprovisionamiento/`) is a `DataTable` with a status filter, received-units progress, a mobile card and `?proveedor=<id>` filtering; `/nuevo` is the canonical create route (`/new` redirects). A draft opens `RestockOrderDraftForm`; an ordered order opens `RestockOrderWorkspace` (step indicator, frozen lines table with the landed cost and totals under the Subtotal column, notes card, receipts list linking to `movimientos-inventario?referencia=<orderId>`, care zone with «Cerrar pedido», «Cancelar pedido», «Volver a borrador», «Eliminar»). The receive dialog proposes the remaining quantity per line, shows the landed cost, and locks its button until the request ends.
- **Suppliers (`Supplier`)** carry `nit`, `contactName`, `phone`, `email`, `leadTimeDays` and `notes`; every payload goes through `parseSupplierInput` in `lib/suppliers.ts` (trims, lowercases the email, keeps digits with an optional leading `+` in the phone). The name is unique per store regardless of case: the API compares lowercased names before writing and maps the `@@unique([storeId, name])` `P2002` to the same Spanish 409. A supplier referenced by products or restock orders cannot be deleted (single or bulk); the 409 names both counts. Supplier reads are owner-only with `NO_CACHE`; the dashboard loaders are plain server functions scoped by `storeId`, never exported server actions, and a foreign id 404s.
...
7. At the end, move the fair to **RECONCILING** («Pasar a conciliación», `POST …/reconcile/start`; «Reabrir ventas» = `…/reconcile/reopen`): sales stop (`createFairSale` requires OPEN) and the count table appears. Closing (`POST …/reconcile`) is only allowed from RECONCILING and is irreversible. Returned stock goes back online through `createInventoryMovementBatchResilient` (a return that cannot land becomes an `OrderInventoryIssue` with `orderId: null` and `orderNumber: "Feria: <name>"`; the fair still closes and the debt shows in Movimientos, where retry writes a `FESTIVAL_RETURN`), damaged/lost stock stays only on the fair item, packed capsules are voided, and affected Mercado Libre listings are refreshed.
8. A fair sale is cancelled only from the fair (`POST …/sales/[orderId]/cancel`, `cancelFairSale`) while OPEN or RECONCILING: it returns the fair counters (`soldQuantity`, a sold capsule back to PACKED), sets the order CANCELLED with `paidAt` cleared and never touches `Product.stock` (the reservation still holds). The generic order PATCH/DELETE routes (single and bulk) refuse `POINT_OF_SALE` and `FESTIVAL` orders (`lib/in-person-orders.ts`), because their restock path would re-add units the fair never subtracted.
...
- The category predictor and attributes endpoints return arrays; use `getMercadoLibreJson`, never the object-only `getMercadoLibreResource`, when consuming them. The wizard loads Mercado Libre's required editable attributes after the category step. Immediately before any direct or queued publication, `validateMercadoLibreListingForPublication` (called by `createMercadoLibreItem`) revalidates that the `MCO` category exists, is final, allows new listings, supports new condition, has all required editable attributes (a required `GTIN` is satisfied by `Product.hasNoProductIdentifier`), and accepts the draft title, marketplace price, and selected-picture count. A draft with no units after the safety buffer is blocked before confirmation. A *review* failure keeps the listing as a reviewable `DRAFT` with `metadata.publicationError` and stops queued publication without retries; a *transient* failure (5xx, 429, timeout) keeps the status and is retried by the outbox; a *reauth* failure (401/403) flags the connection `REAUTH_REQUIRED`. None of them may create an external item. See «Publication guards and wizard resilience» below.
- A category ID is never trusted only because it has the `MCO...` shape. Predictor results are checked against the live category resource before they are shown, and draft/profile/template writes verify both the category and its attributes endpoint. If Mercado Libre retires a category or returns `404`, the wizard clears only that stale category, keeps the rest of the draft including the technical sheet already typed (the new category's attributes are prefilled only where empty), and loads verified alternatives. The category search (`GET /marketplaces/mercadolibre/categories`) inspects the predictor's candidates at most 3 at a time (`MAX_CONCURRENT_CATEGORY_INSPECTIONS`, `lib/concurrency.ts › mapWithConcurrency`), attaches each category's `path_from_root` names as `path`, drops non-publishable ones silently and answers `{ suggestions, unavailableCount }` so the wizard can say how many Mercado Libre could not verify; it still fails when nothing could be verified and always answers 401 on an expired authorization. Authentication and temporary upstream errors stay distinct and must not be presented as an invalid product choice. AI may improve the human-readable family name/search phrase, but it must never invent or act as the authority for a Mercado Libre category ID.
...
- The price proposal may target the unit cost floor (acquisition cost plus `Product.transportationCost`) plus the configured profit after Mercado Libre's current estimated sale fee and listing fee (`listingFeeAmount` from `listing_prices`). When the seller offers free shipping, it must also include the official estimated seller shipping cost and must refuse to calculate until package dimensions/weight have been evaluated; buyer-paid shipping is never counted as a seller cost. It still excludes taxes, refunds, campaigns, and final-settlement adjustments; label it as an estimate, not net income or a guaranteed margin.
- The listing form is a concise four-step wizard: product/price, category/photos, technical attributes, and final review. It validates each step field by field (`lib/mercadolibre/listing-wizard.ts › getListingWizardStepIssue`, inline `role="alert"` message plus focus), loads required attributes before the technical step, and can save a draft or publish directly after a single explicit confirmation («¿Guardar y publicar?»). Since 2026-09 it is draft-as-you-go: leaving step 1 creates the draft (`POST …/listings` without price or category, so the below-cost guard cannot fire early), leaving steps 2 and 3 PATCHes category/photos/name and attributes, the final save PATCHes that same draft (never a second `POST`), closing while dirty asks for confirmation, and reopening a draft lands on the first incomplete step or on the step Mercado Libre rejected (`getInitialListingWizardStep`). Attributes and fees are refetched only when category or price changed. BRAND/GTIN/MPN/COLOR/SIZE are prefilled from the product only into empty fields and, for closed lists, only on an exact match (`prefillListingAttributes`); a list attribute may take a free value through «Ninguna coincide: escribir otro valor». UI kit: `SectionCard` per step, `RadioCards` for listing type and shipping, `Label required` asterisks (never «(opcional)»), the dialog title names the current step and the step name is visible at phone width.
...
**Publication guards and wizard resilience (2026-09).**

- **Exactly-once publish.** `POST …/listings/[listingId]/publish` no longer calls Mercado Libre itself: it queues (or reuses) the `PUBLISH_LISTING` outbox event keyed by `getMarketplaceListingPublicationKey(connectionId, listingId)`, refuses with 409 while that event is `PROCESSING`, and runs `processMarketplaceOutboxEvent` inline, mapping `processed|superseded` → 201, `claimed_elsewhere|not_due` → 409, `listing_requires_review|failed` → 400 with `lastError`, `retry_scheduled` → 400 with «se reintentará automáticamente». The outbox branch persists `externalItemId`, status and `lastSyncedStock` immediately after `POST /items` (`createMercadoLibreItem`), queues a stock sync, and only then sends the description as a best-effort step (`createMercadoLibreItemDescription`, failure only writes `lastError`). A listing that already has an id completes the event as a no-op and reconciles `DRAFT|ERROR` → `ACTIVE`. Post-create statuses map identically in both paths (`getMarketplaceListingStatusFromRemote`). `publishMercadoLibreListing` is deprecated.
- **Failure classification.** `lib/mercadolibre/publication-error.ts › mapMercadoLibreItemError(status, payload)` turns a Mercado Libre rejection into `{ kind: "review" | "transient" | "reauth", step: "producto" | "categoria" | "ficha" | "precio", field, message, code }` (attribute before category, pictures index, price, listing type, sale terms, family name, title, quantity, condition; 5xx/429/408 transient; 401/403 reauth). `MercadoLibrePublicationError` carries it; the outbox stores it in `metadata.publicationError { kind, step, field, message, code, at }` (`listing-metadata.ts › withMercadoLibrePublicationFailure`). Review → `DRAFT`, no retry; transient → status kept, retried up to `MAX_OUTBOX_EVENT_ATTEMPTS` (12) by the 15-minute recovery schedule; reauth → connection `REAUTH_REQUIRED`; exhausted → `ERROR`. The manager reads `publicationError.step/field` to reopen the wizard on the rejected step with the field flagged.
- **Below-cost guard.** `lib/product-costs.ts › getUnitCostFloor(product)` = `acqPrice + transportationCost` (null when no acquisition cost). `lib/mercadolibre/listing-price-guard.ts › evaluateListingPrice` refuses a marketplace price below that floor on `POST`/`PATCH …/listings` (400, `details.code = PRICE_BELOW_COST`) unless the body carries `priceOverride: { reason }` (5–200 chars, `parsePriceOverride`), in which case `metadata.belowCostOverride { reason, floor, price, at }` is stored; `buildItemPayload` refuses an unauthorized below-cost price at publish time too. `lib/mercadolibre/health.ts` `margin_risk` uses the same floor. `Product.transportationCost` («Envío y otros gastos» in Productos › Precios, `parseTransportationCost`) is persisted since 2026-09-11 and defaults to 1000 for the pre-existing catalog (backfill above); it is not a shipping quote and never reaches the storefront.
- **Package dimensions** from the wizard go to Mercado Libre as `shipping.dimensions` («HxWxL,g») plus `PACKAGE_HEIGHT/WIDTH/LENGTH/WEIGHT` attributes so the seller shipping estimate matches the comparison shown before publishing.

**Managing existing listings (2026-09).** The Publicaciones tab is `mercadolibre/components/listings/` (`listing-table.tsx` on the shared `DataTable`, `listing-row-actions.tsx`, `listing-details-sheet.tsx`, `listing-signals.ts`, `listing-types.ts`) driven by `listing-manager.tsx`, which keeps state and fetches and hands the memoized table stable handlers through a ref. Rules that hold across the eight listing routes: every mutation is queued through the outbox (`bulk` returns `results[]` per listing; per-row pause/activate call `bulk` with one id); bulk `publish` accepts only DRAFT/ERROR listings with no `metadata.publicationError.kind === "review"` and skips one whose PUBLISH_LISTING event is PROCESSING; `PATCH …/listings/[id]` on a published listing queues a content sync for category/attributes/images/familyName and a stock sync when `stockSafetyBuffer` changes, refuses when the connection or QStash gate fails, and clears `publicationError` when the edited field belongs to the rejected step; DELETE refuses while a publication is PROCESSING; `SYNC_LISTING_STATUS` stores the status Mercado Libre returned, mapped by the shared table; content-sync failures write `lastError`. `metadata` is written only through `buildMercadoLibreListingMetadata` (merge, never replace; unknown keys survive; `version` increments) and, from the panel, through `updateMarketplaceListingMetadataGuarded` (409 «La publicación cambió mientras la editabas» when the stored version moved). `GET …/sale-conditions` never writes; its PATCH persists the real `shippingMode`. Import (`lib/mercadolibre/import-listings.ts`): preview crawls the account with `getRemoteListingsByIds` (batches of 20, 3 in flight, failed batches counted in `summary.unavailable`/`partial`, 401 stops), uses `getMarketplaceListingStatusFromRemote`, and flags `draftListingId`, non-COP `currencyId`, `catalogListing` and status notes in `warnings`; the import re-reads only the selected ids, refuses to overwrite a draft unless `replaceDraft` is true, names the offending item in every error and returns `imported[]`. Mercado Libre client failures are `MercadoLibreRequestError` (401 reauth, 404, 429, 502 outage, 400 other) with `details.upstreamStatus/upstreamMessage`; the manager appends the upstream text to the banner. Tests: `tests/unit/routes/mercadolibre-listings-bulk.test.ts`, `mercadolibre-listing-detail.test.ts`, `mercadolibre-listing-insights-routes.test.ts`, `tests/unit/lib/mercadolibre-listing-metadata.test.ts`, `mercadolibre-client-errors.test.ts`, `mercadolibre-import-listings.test.ts`, `tests/components/listing-table.test.tsx`, `listing-manager.test.tsx`, and the listing routes in the authorization table. Still open: list attributes store display names (value ids only round-trip when unchanged) and lists are cut at 100 values; the video-rule detection is a wording heuristic; the storefront-facing sale-conditions financial preview is not re-validated server-side.
...
A sale item's `listingId`/`productId` are resolved **once**, at synchronization time, by matching `(externalItemId, externalVariationId)` against `MarketplaceListing` for that connection — never by SKU. A sale that arrives before its publication is imported/linked, or whose variation id does not match, is stored permanently unlinked: no acquisition cost in reports and `inventoryStatus = EXCEPTION` with inventory never applied. The repair is **Re-sincronizar** on the sale's row in Ventas (also in Rentabilidad; `POST /api/[storeId]/marketplaces/mercadolibre/orders/[externalOrderId]/resync`), which reads the order from Mercado Libre FIRST and only then releases an `EXCEPTION` order back to `NOT_APPLIED` (a failed fetch leaves it visibly in EXCEPTION), then rebuilds its items against today's publications. Items are rebuilt only while `inventoryStatus` is `NOT_APPLIED` or `EXCEPTION`; once inventory is applied a sale's items are a historical record and later webhooks never recreate them. `DECREMENTED` and `RESTOCK_PENDING` are left untouched so units are never discounted twice. Reconciliation of historical sales cannot repair this: it refuses an order that already exists and points to resync.

**Cancelled or refunded sales (2026-09).** A cancellation or full refund never restocks on its own: a `DECREMENTED` sale becomes `RESTOCK_PENDING`, and the human step is **Confirmar retorno físico** on the row (`POST …/orders/[externalOrderId]/restock`, `lib/mercadolibre/order-restock.ts`): atomic claim `RESTOCK_PENDING → RESTOCKED`, kit explosion via `explodeSaleLines`, returns written through `createInventoryMovementBatchResilient` as `ORDER_CANCELLED` movements referencing the marketplace order, failed lines recorded as `OrderInventoryIssue` with `orderId: null` and `orderNumber: "Mercado Libre: <id>"`, stock re-sync queued. Full return of every unit; no partial variant.

**Refunds and statuses (2026-09).** `MarketplaceOrderStatus` has `PARTIALLY_REFUNDED` and `REFUNDED`; `MarketplaceOrder.refundedAmount` sums `payments[].transaction_amount_refunded` (migration `20260911_add_marketplace_order_refunds.sql`, applied to Railway 2026-09-11). Mapping in `order-sync.ts › getMarketplaceOrderStatus`: `cancelled|invalid` → CANCELLED; any payment `charged_back` → REFUNDED; `partially_refunded` → PARTIALLY_REFUNDED; `pending_cancel` → REFUNDED; `paid` → PAID even with a refund (recorded in `refundedAmount` and `metadata.refund`); everything else → PENDING. Net = `max(0, total − charges − taxes − refunded)`; a change in the refunded amount re-opens the financials job. Every money query uses `REVENUE_MARKETPLACE_ORDER_STATUSES` (PAID, PARTIALLY_REFUNDED) from `lib/mercadolibre/order-status.ts`; REFUNDED is not revenue and behaves like a cancellation for stock. Never filter on `PAID` alone.
...
- **Queue hygiene (2026-09).** Webhook and outbox events give up after `MAX_WEBHOOK_EVENT_ATTEMPTS` / `MAX_OUTBOX_EVENT_ATTEMPTS` (12) and become `FAILED`, which is what the `webhook_failed` / `outbox_failed` health kinds read; a pending settlement is exempt (6 h cadence). Claims honour `nextRetryAt` / `availableAt`: an early redelivery answers 200 `not_due`, a failed attempt writes RETRY plus the backoff and answers 200 `retry_scheduled`, and the 15-minute recovery schedule performs the retry (QStash's immediate retries are no longer relied on). Outbox events stuck in `PROCESSING` for 15 minutes are swept back to RETRY like webhook events. A `SYNC_STOCK` job pushes the live stock read at processing time and completes only while still `PROCESSING` and the stock has not moved; otherwise it stays open and is re-enqueued (`reason: "superseded"`), so a stock change during a sync can no longer be overwritten with a stale quantity.
...
- The administrative sale email is queued inside the inventory transaction, keyed per sale (`SEND_ORDER_NOTIFICATION` dedupe key), so a retry after a crash neither loses nor duplicates it; later `orders_v2` or shipment updates for an already-paid order cannot create a delayed "new sale" notification because the key already exists. Include the recorded payment date so the email remains accurate if delivery is later.
...
| Revalidation alert says `429` / the store logs show no `/api/revalidate` call | Vercel Firewall on `pdepapel-store` (Bot Protection «Challenge» or Attack Challenge Mode) is challenging the admin's server-to-server call | Add a Bypass custom rule for `/api/revalidate` or set Bot Protection to «Log» in the store's Firewall; the alert detail now names the mitigation header. |
...
| Mercado Libre listing back to DRAFT with a message | Mercado Libre rejected a field (`metadata.publicationError.kind = review`) | Open **Editar**: the wizard lands on the rejected step with the field flagged. Fix it and publish again; never retry blindly.    |
| Listing publish says «se reintentará automáticamente» | Transient Mercado Libre failure (5xx/429/timeout); outbox event `retry_scheduled` | Wait for the 15-minute recovery run; do not click publish again (the outbox claim prevents a duplicate item anyway).          |
| `PRICE_BELOW_COST` on saving a listing   | Marketplace price under `acqPrice + transportationCost`               | Raise the price or send `priceOverride.reason` from the wizard's step 4; check «Envío y otros gastos» on the product.           |
