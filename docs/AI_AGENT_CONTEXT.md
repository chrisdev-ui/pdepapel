...
- **Google Merchant feed** (`npm run export:products-merchant` in `pdepapel-admin`, helpers in `lib/google-merchant.ts`): exports only non-archived products, verifies every storefront `link` before writing the feed (unreachable pages are excluded and listed in the sibling `google_merchant_report_*.txt`; redirected links are exported with their final URL), re-requests Cloudinary images in unsupported formats (WebP/AVIF) as PNG for `image_link`, and sets `excluded_destination` to the local destinations because the business has no physical store. Each run deletes the previous feed/report pair so only the latest export remains; both files are gitignored and the feed is uploaded manually to Merchant Center. Keep local listings disabled in the Merchant Center account.
