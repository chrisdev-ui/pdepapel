...
- The image-analysis route accepts only HTTPS Cloudinary catalog URLs, at most three images per request, and uses Upstash Redis to enforce 20 new model requests per store per Colombia calendar day. A 24-hour cache is keyed by the exact public photos, selected category, and current category/size/color/design taxonomy; a cache hit is re-sanitized against the current taxonomy and does not consume another daily analysis. It can propose one recommended factual base name plus up to two equivalent customer-search-friendly alternatives, a plain-text description draft, visible brand, and category/size only through one unambiguous exact match to an existing local option. Titles prioritize natural Colombian commercial wording (for example, `mouse pad`, never `pad mouse`), keep the product type first, and may include only visible descriptors shared by every unit. A deterministic color/design matching the store taxonomy is marked `existing`; a visible, reusable value not found in the taxonomy is marked `new` and stays a proposal. The administrator may explicitly confirm **Crear y usar** in the individual product form; only then does the normal authenticated Color/Design API create it and select it. Product-group forms never auto-create variant attributes from a group photo.
- GTIN candidates require both clearly visible digits and a valid GS1 check digit; MPN candidates require a complete, visibly read manufacturer reference. Neither is loaded by **Cargar propuesta**: each has its own human confirmation dialog and is still saved only through the regular form. A variant suggestion is informational; it can only open the existing conversion review flow for an eligible individual product, never create or alter variants. No model output may create a SKU, price, stock change, slug change, or automatic Mercado Libre update.
