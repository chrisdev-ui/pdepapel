...
- **Commit messages:** Spanish conventional commits (`fix(whatsapp): …`), one subject line that says what changed for the business.
...
- **When a change touches authentication or authorization, run two checks before asking for approval:** every page or route that now calls `auth()`/`currentUser()` is in the middleware's route list, and the assumption the change rests on is confirmed with a read-only query against production data rather than read off the schema. A page that calls Clerk outside its matcher answers 500, and the data has contradicted the schema before: many production orders carry the owner's `userId` because the owner registers WhatsApp orders on the customer's behalf.
