 ...
 */
export function CategoryChips({ types, limit = 5, activeTypeId, className }: CategoryChipsProps) {
  const ordered = activeTypeId ? [...types].sort((a, b) => Number(b.id === activeTypeId) - Number(a.id === activeTypeId)) : types;
  const visible = ordered.slice(0, limit);
  if (visible.length === 0) return null;

  return (
    <nav aria-label="Atajos de categorías" className={cn("category-chips flex gap-2 overflow-x-auto px-3 py-2.5 sm:px-6 lg:hidden", className)}>
      {visible.map((type, index) => {
        ...
        return (
          <Link
            ...
            onClick={() => trackCustomerEvent("select_content", { content_type: "category_chip", item_id: type.slug || type.id, item_category: label })}
            className={cn(
              ...
              active && "font-semibold ring-2 ring-blue-yankees ring-offset-1",
            )}
          >
