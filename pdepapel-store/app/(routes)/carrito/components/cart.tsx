...
const Cart: React.FC<CartProps> = ({ suggestions = [], categories = [] }) => {
    ...
    fetchCatalogFromClient({ ids }, controller.signal)
      .then(({ products }) => {
        ...
        products.forEach((product) => {
          ...
          if (before !== undefined && before !== Number(product.price))
            changes[product.id] = { from: before, to: Number(product.price) };
  ...
  // Una preventa no está «agotada» por tener 0 en bodega: lo que la frena es
  // quedarse sin cupo. Ese es el número que se compara aquí.
  const checkoutBlocker = cart.items.find((item) => {
    const units = getPurchasableUnits(item);
    return units === 0 || (item.quantity ?? 0) > units;
  });
  const blockerUnits = checkoutBlocker
    ? getPurchasableUnits(checkoutBlocker)
    : 0;
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        {!isEmpty && (
          <Link
            ...
            className="inline-flex items-center gap-1.5 font-sans text-sm font-semibold text-blue-yankees underline underline-offset-4"
          >
      ...
      {isEmpty ? (
        ...
        <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_400px] lg:items-start lg:gap-12">
          ...
          <Summary
            disabledReason={
              checkoutBlocker
                ? `Revisa «${checkoutBlocker.name}»: ${
                    blockerUnits === 0
                      ? isPresaleItem(checkoutBlocker)
                        ? "se acabaron las reservas"
                        : "está agotado"
                      : isPresaleItem(checkoutBlocker)
                        ? `solo quedan ${blockerUnits} reservas`
                        : `solo quedan ${blockerUnits}`
                  }.`
                : null
            }
          />
        ...
        <ProductList
          ...
          eyebrow={
            !isEmpty && freeShippingRemaining > 0
              ? `Te faltan ${currencyFormatter.format(freeShippingRemaining)} para el envío gratis`
              : "Lo más pedido"
          }
  ...
  return (
    <div className="flex flex-col items-center gap-4 rounded-2xl border-2 border-dashed border-pink-shell bg-kawaii-pink-light/25 px-6 py-10 text-center">
      ...
      <p className="max-w-md font-sans text-sm text-gray-600">
        ...
        {threshold
          ? ` Envío gratis desde ${currencyFormatter.format(threshold)}.`
          : ""}
      </p>
      {categories.length > 0 && (
        <ul
      ...
      <div className="flex flex-wrap justify-center gap-2.5">
        ...
        {wishlistCount > 0 && (
          <Button
            asChild
            variant="outline"
            className="h-11 rounded-full border-2 border-blue-yankees px-5 font-sans font-semibold text-blue-yankees"
          >
