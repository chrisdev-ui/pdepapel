 ...
 * Consume la aprobación ANTES de ejecutar (un fallo a medias no la deja
 * reutilizable), pasa `DATABASE_URL` sólo al proceso hijo y anota la corrida en
 * `ops/prod-writes.log`, que se versiona con el repositorio.
 */
...
// Se consume antes de correr: si el guion falla a medias, la siguiente
// corrida vuelve a necesitar una aprobación nueva, con un motivo nuevo.
...
console.log(`prod-write: ${scriptPath}@${scriptHash} → ${target}`);
console.log(`prod-write: motivo «${approval.reason}» (aprobado por ${approval.operator ?? "?"}, vence ${approval.expiresAt})`);

const result = spawnSync(process.execPath, [absoluteScript, ...scriptArgs], {
  cwd: projectRoot,
  env: {
    ...process.env,
...
appendFileSync(
  ...
  formatLogLine({
    ...
    scriptHash,
    target,
    exitCode: result.status ?? "signal",
    rowsAffected: rowsMatch ? Number(rowsMatch[1]) : null,
  }),
);
console.log(`prod-write: registrado en ${PROD_WRITE_LOG} (recuerda incluirlo en el commit).`);
