...
vi.mock("@/lib/prismadb", () => ({
  default: {
    productPresale: { findFirst: mocks.presaleFindFirst, update: mocks.presaleUpdate },
    orderItem: { findMany: mocks.itemFindMany, updateMany: mocks.itemUpdateMany },
    product: { findFirst: mocks.productFindFirst },
    $transaction: mocks.transaction,
  },
}));
vi.mock("@/lib/order-stock-movements", () => ({ explodeKitMovements: mocks.explode }));
...
const input = { storeId: "store-1", presaleId: "presale-1", releasedBy: "user-paula" };
...
function presale(overrides: Record<string, unknown> = {}) {
  return {
    ...
    unitLimit: 40,
    committedUnits: 3,
    product: { id: "product-1", name: "Agenda", sku: "AGE-01", stock: 40, isKit: false },
    ...overrides,
  };
}

const LINES = [
  { id: "i1", orderId: "order-a", quantity: 2, productId: "product-1", order: { id: "order-a", orderNumber: "ORD-1", status: "PAID", fullName: "Laura" } },
  { id: "i2", orderId: "order-b", quantity: 1, productId: "product-1", order: { id: "order-b", orderNumber: "ORD-2", status: "PAID", fullName: "Ana" } },
];
...
describe("releasePresale", () => {
  ...
  it("marca las líneas como liberadas: eso es lo que suelta el pedido completo", async () => {
    ...
    expect(mocks.itemUpdateMany).toHaveBeenCalledWith({
      where: { presaleId: "presale-1", isPreorder: true, preorderReleasedAt: null },
  ...
  it("se niega si no hay mercancía suficiente en bodega", async () => {
    mocks.presaleFindFirst.mockResolvedValue(presale({
      product: { id: "product-1", name: "Agenda", sku: "AGE-01", stock: 2, isKit: false },
    }));
  ...
  it("no libera nada si una sola línea falló al descontar", async () => {
    ...
    mocks.batch.mockResolvedValue({ success: [{}], failed: [{ productId: "product-1" }] });
  ...
  it("se niega cuando no queda nada por liberar", async () => {
    ...
    await expect(releasePresale(input)).rejects.toThrow(/no tiene pedidos pendientes/);
  });
});
