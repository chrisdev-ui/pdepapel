...
vi.mock("@/lib/mercadolibre/questions", () => ({
  synchronizeMercadoLibreQuestion: vi.fn(),
}));
...
describe("processMercadoLibreWebhookEvent", () => {
  ...
  it("honours nextRetryAt: an early redelivery is answered without touching the event", async () => {
    ...
    expect(mocks.claimRow).not.toHaveBeenCalled();
  ...
  it("schedules a retry with a future nextRetryAt instead of throwing", async () => {
    ...
    mocks.getResource.mockRejectedValueOnce(
      new Error("Mercado Libre respondió 429"),
    );
    await expect(processMercadoLibreWebhookEvent("event-1")).resolves.toEqual({
      processed: false,
    ...
    expect(call.data.nextRetryAt.getTime()).toBeGreaterThan(
      Date.now() + 4 * 60 * 1000,
    );
  ...
  it("moves the event to FAILED once the attempts are exhausted", async () => {
    mocks.findEvent.mockResolvedValue(
      event({ status: "RETRY", attempts: MAX_WEBHOOK_EVENT_ATTEMPTS - 1 }),
    );
    mocks.getResource.mockRejectedValueOnce(
      new Error("La orden de Mercado Libre no contiene productos"),
    );
    await expect(processMercadoLibreWebhookEvent("event-1")).resolves.toEqual({
      processed: false,
      reason: "failed",
