...
export async function POST(req: Request, { params }: { params: Params }) {
  ...
  try {
    ...
    const banner = await createNewBanner({ ...body, storeId: params.storeId });
    return NextResponse.json(banner);
  } catch (error) {
    console.log("[BANNERS_POST]", error);
    return handleErrorResponse("[BANNERS_POST_ERROR]", 500);
  }
}
...
export async function GET(_req: Request, { params }: { params: Params }) {
  if (!params.storeId) return handleErrorResponse("Store ID is required", 400);
  try {
    const banners = await getBannersByStoreId(params.storeId);
    return NextResponse.json(banners);
  } catch (error) {
    console.log("[BANNERS_GET]", error);
    return handleErrorResponse("[BANNERS_GET_ERROR]", 500);
  }
}
