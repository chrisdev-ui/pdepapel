...
// producto, más la nota media y el total.
export async function GET(req: Request, { params }: { params: { storeId: string } }) {
  try {
    ...
    const where = { storeId: params.storeId, ...PUBLIC_REVIEW_WHERE, product: { isArchived: false } };
    ...
    const [reviews, summary] = await Promise.all([
      prismadb.review.findMany({
        ...
        select: {
          ...
          product: {
            select: { id: true, name: true, slug: true, images: { where: { isMain: true }, take: 1, select: { url: true } } },
          },
        },
      }),
    ...
    return NextResponse.json(
      {
        reviews: reviews.map(({ product, ...review }) => ({
          ...review,
          product: { id: product.id, name: product.name, slug: product.slug, imageUrl: product.images[0]?.url ?? null },
        })),
