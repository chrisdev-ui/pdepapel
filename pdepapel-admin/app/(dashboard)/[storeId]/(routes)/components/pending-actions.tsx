...
const ICONS: Record<TodayPendingAction["kind"], { icon: React.ReactNode; tint: string }> = {
  ...
  "payment-webhook-issue": { icon: <AlertTriangle className="h-[18px] w-[18px]" aria-hidden="true" />, tint: "bg-tint-pink" },
          ...
          {items.map((item, index) => {
            ...
            return (
              <li key={`${item.kind}-${index}`} className="flex items-center gap-3 px-4 py-3">
                ...
                <Button asChild size="xs" variant={item.kind === "verify-payment" || item.kind === "awaiting-payment" || item.kind === "inventory-issue" || item.kind === "shipping-issue" || item.kind === "payment-webhook-issue" ? "default" : "outline"}>
