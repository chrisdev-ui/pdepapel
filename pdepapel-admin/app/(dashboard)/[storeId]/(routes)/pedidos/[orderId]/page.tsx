...
export default async function OrderPage({ params }: { params: { orderId: string; storeId: string } }) {
  const [{ order, products, categories }, coupons, users, locations, boxes, storeSettings] = await Promise.all([
    ...
    prismadb.store.findUnique({ where: { id: params.storeId }, select: { freeShippingThreshold: true } }).catch(() => null),
  ]);
  ...
  return (
    <div className="flex-col">
      <div className="flex-1 space-y-4 p-4 sm:p-8 sm:pt-6">
        {order && <OrderWorkspaceHeader storeId={params.storeId} order={order} />}
        <OrderForm
          ...
          shippingInfo={order?.shipping ? <ShippingInfo shipping={order.shipping} orderStatus={order.status} freeShipping={freeShipping} /> : null}
        />
      </div>
    </div>
  );
