...
export function ItemsSection({
  ...
  const itemsErrors = form.formState.errors.orderItems as
          ...
          {fields.map((field) => (
            <li key={field.id} className="flex items-center gap-3 p-3">
              <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-md border bg-muted">
                <Image
                  src={field.imageUrl || "/images/placeholder_1.png"}
                  alt=""
                  fill
              ...
              <span className="text-sm font-semibold text-primary">
                {currencyFormatter(
                  Number(field.price) * Number(field.quantity),
                )}
              </span>
            </li>
          ))}
                  ...
                  onUpdate={(productId, quantity, product) => {
                    ...
                    if (quantity > 0) {
                      const availableStock =
                        product?.stock ?? fields[existingIndex]?.stock ?? 9999;
                      if (availableStock <= 0) {
                        toast({
                          variant: "destructive",
                          title: "Producto sin stock",
                          description: `«${product?.name || "El producto"}» está agotado.`,
                        });
                        return;
                      }
                  ...
                  {fields.map((field, index) => {
                    if (!field.productId) {
                      ...
                      return (
                        <div
                          key={field.id}
                          className="relative rounded-lg border border-dashed bg-muted/20 p-4"
                        >
                          <div className="absolute right-2 top-2 flex gap-1">
                            <Button
                              ...
                              title="Convertir en producto del catálogo"
                              aria-label="Convertir en producto del catálogo"
                            >
                          ...
                          <div className="grid gap-4">
                            ...
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                              <div className="space-y-1">
                                <Label
                                  htmlFor={`item-${field.id}-name`}
                                  className="text-xs"
                                >
                                  Nombre
                                </Label>
                                <Input
                                  id={`item-${field.id}-name`}
                                  {...form.register(`orderItems.${index}.name`)}
                            ...
                            <div className="grid grid-cols-[1fr_auto] items-end gap-4">
                              ...
                              <div className="space-y-1">
                                <Label
                                  htmlFor={`item-${field.id}-cantidad`}
                                  className="text-xs"
                                >
                                  Cantidad
                                </Label>
                                <StockQuantityInput
                                  id={`item-${field.id}-cantidad`}
                                  ...
                                  min={1}
                                  ariaLabel={`Cantidad de ${itemValues.name || "ítem manual"}`}
                                  className="w-auto"
                                />
                              </div>
                            </div>
