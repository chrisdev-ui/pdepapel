...
import { ORDER_STATUS_LABELS, ORDER_STATUS_TONE } from "@/lib/order-transitions";
...
export function OrderStatusBar({
  ...
  const shared = {
    ...
    trackingCode, transactionId, guideRate, hasGuide, loading, onTransition,
  };
  ...
  return (
    <section
      ...
      className={cn("flex scroll-mt-24 flex-col gap-3 rounded-xl border p-4 sm:px-5", TONE_BG[tone])}
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 flex-col gap-1">
          <div className="flex flex-wrap items-center gap-2">
            <TintBadge label={ORDER_STATUS_LABELS[status]} tone={ORDER_STATUS_TONE[status]} />
            <h2 id="estado-del-pedido-titulo" className="text-[15px] font-bold text-primary">
              {nextStep?.title ?? "Sin nada pendiente"}
            </h2>
          </div>
          {nextStep?.description && <p className="text-sm text-primary/90">{nextStep.description}</p>}
          {nextStep?.consequence && <p className="text-xs text-primary/70">{nextStep.consequence}</p>}
        </div>
      ...
      {steps.length > 0 && (
        ...
        >
          {steps.map((step, index) => (
            <li key={step.id} className="flex min-w-[112px] flex-1 flex-col gap-1.5">
              <div className="flex items-center gap-1.5">
                ...
                {index < steps.length - 1 && (
                  <span className={cn("h-0.5 flex-1", step.state === "done" ? "bg-[#B8E8C8]" : "bg-primary/10")} aria-hidden="true" />
                )}
              </div>
              <span className={cn("text-[13px] font-semibold", step.state === "todo" || step.state === "skipped" ? "text-primary/60" : "text-primary")}>
                {step.label}
                <span className="sr-only">{step.state === "done" ? " (hecho)" : step.state === "now" ? " (en curso)" : ""}</span>
              </span>
              {step.meta && <span className="truncate text-xs text-primary/60" title={step.meta}>{step.meta}</span>}
            </li>
          ))}
        </ol>
      )}
