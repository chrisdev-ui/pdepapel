...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onCopy = (id: string) => {
    ...
    toast({
      description: "ID del color copiado al portapapeles",
      variant: "success",
    });
  };
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/${Models.Colors}/${data.id}`);
      router.refresh();
      toast({
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/${params.storeId}/${Models.Colors}/${data.id}`)
            }
          >
            <Edit className="mr-2 h-4 w-4" />
            Actualizar
          </DropdownMenuItem>
          <ArchiveMenuItem kind="colors" row={data} />
          <DropdownMenuItem
