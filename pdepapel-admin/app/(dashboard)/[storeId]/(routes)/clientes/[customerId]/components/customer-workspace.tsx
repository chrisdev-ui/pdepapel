...
import {
  CHANNEL_TONE,
  TintBadge,
} from "../../../pedidos/components/order-badges";
...
const ORDER_STATUS: Partial<
  Record<OrderStatus, { label: string; tone: string }>
> = {
  ...
  const kpis = [
    {
      ...
      value: String(customer.paidOrders),
      hint: `${customer.totalOrders} pedidos en total`,
    },
    {
      label: "Gastado",
      value: currencyFormatter(customer.totalSpent),
      hint:
        customer.averageOrderValue > 0
          ? `Ticket ${currencyFormatter(customer.averageOrderValue)}`
          : "Sin compras pagadas",
    },
    {
      label: "Unidades",
      value: String(customer.totalItems),
      hint: customer.favoriteProducts[0]
        ? `Prefiere ${customer.favoriteProducts[0].name}`
        : "—",
    },
    {
      label: "Última compra",
      value: customer.lastPaidAt ? relativeDate(customer.lastPaidAt) : "Nunca",
      hint: customer.lastPaidAt
        ? DATE.format(customer.lastPaidAt)
        : `Cliente desde ${DATE.format(customer.firstOrderAt)}`,
    },
  ];
        ...
        <div className="flex min-w-0 flex-col gap-1.5">
          ...
          {customer.segment === "inactivo" && inactiveDays !== null && (
            <p className="text-sm text-muted-foreground">
              Lleva {inactiveDays} días sin comprar. Un mensaje corto con
              novedades suele traerle de vuelta.
            </p>
          )}
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {kpis.map((kpi) => (
          <div key={kpi.label} className="rounded-xl border bg-white p-4">
            ...
            <p className="mt-1 truncate text-2xl font-bold tabular-nums text-primary">
              {kpi.value}
            </p>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        <Card className="min-w-0">
          <CardHeader>
            ...
            <CardDescription>
              Todos los pedidos hechos con este teléfono, del más reciente al
              más antiguo. La ficha se arma agrupando por número, no por
              persona.
            </CardDescription>
          </CardHeader>
                ...
                return (
                  <li key={order.id}>
                    <Link
                      ...
                      className="flex min-w-0 items-center gap-3 px-6 py-3 transition-colors hover:bg-accent/50"
                    >
        ...
        <div className="flex flex-col gap-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Correos automáticos a los 90 días sin comprar.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {reactivations.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  {customer.email
                    ? "Sin correos de reactivación todavía."
                    : "Sin correo registrado: solo se puede escribir por WhatsApp."}
                </p>
              ) : (
                <ul className="space-y-2">
                  {reactivations.map((item) => (
                    ...
                    >
                      ...
                      <span className="shrink-0 text-xs text-muted-foreground">
                        {relativeDate(item.sentAt ?? item.createdAt)}
                      </span>
                    </li>
                  ))}
                </ul>
