...
import { TruncationNote } from "./customer-notes";

const CARDS = [
  {
    key: "total",
    label: "Clientes",
    hint: "Personas con al menos un pedido registrado",
  },
  {
    key: "buyers",
    label: "Han comprado",
    hint: "Con al menos una compra pagada",
  },
...
export function CustomerOverviewPanel({
  ...
  const { summary, cities, truncated } = overview;
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <CardContent>
          {cities.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              Todavía no hay ciudades registradas en los pedidos.
            </p>
          ) : (
                ...
                <li key={row.city} className="flex items-center gap-3">
                  ...
                  <span className="h-2 flex-1 overflow-hidden rounded-full bg-muted">
                    <span
                      ...
                      style={{
                        width: `${topCity > 0 ? Math.max(4, Math.round((row.customers / topCity) * 100)) : 0}%`,
                      }}
                    />
                  </span>
      ...
      <p className="text-xs text-muted-foreground">
        La lista de clientes, con nombre y contacto, solo la ve la dueña de la
        tienda.
      </p>
    </div>
  );
