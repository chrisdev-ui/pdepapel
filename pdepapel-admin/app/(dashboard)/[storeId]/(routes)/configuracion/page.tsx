...
export const metadata: Metadata = {
  ...
  description:
    "Tienda, envíos y empaques, pagos, integraciones y herramientas avanzadas",
};
...
export default async function SettingsPage({
  ...
  const tab: Tab = TABS.some((item) => item.id === searchParams.tab)
    ? (searchParams.tab as Tab)
    : "tienda";
  const hrefFor = (id: Tab) =>
    `/${params.storeId}/configuracion${id === "tienda" ? "" : `?tab=${id}`}`;
    ...
    <div className="flex flex-col gap-4 p-4 sm:p-8 sm:pt-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          Datos de la tienda, envíos y empaques, cómo se cobra, qué servicios
          están conectados y las herramientas que rara vez se tocan.
        </p>
      </div>
      <nav
        role="tablist"
        ...
        <div className="flex flex-col gap-8">
          ...
          <BoxesPanel
            data={(await getBoxes(params.storeId)).map((box) => ({
              ...box,
              dimensions: `${box.width} x ${box.height} x ${box.length}`,
            }))}
          />
        </div>
      )}
      ...
      {tab === "integraciones" && (
        <IntegrationsPanel storeId={params.storeId} />
      )}

      {tab === "avanzado" && (
        <div className="flex flex-col gap-6">
          <p className="text-sm text-muted-foreground">
            Herramientas para quien desarrolla o mantiene la tienda. No hacen
          ...
          <section className="rounded-xl border bg-white p-4">
            <h2 className="mb-1 text-base font-semibold text-primary">
              Caché de cotizaciones de EnvioClick
            </h2>
            <p className="mb-4 text-sm text-muted-foreground">
              Cotizaciones guardadas para no repetir llamadas. Limpiarla es
              seguro; solo vuelve a cotizar.
            </p>
            <CacheManagement />
          </section>
        </div>
