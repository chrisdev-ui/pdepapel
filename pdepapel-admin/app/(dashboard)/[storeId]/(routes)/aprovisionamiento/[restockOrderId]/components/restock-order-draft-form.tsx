...
/**
 * El mismo esquema que valida la API, sin `status`: el formulario no lo elige,
 * lo deciden «Guardar borrador» y «Pedir al proveedor». Antes había una copia
 * aquí con los mismos campos y otros mensajes.
 */
const formSchema = restockOrderInputSchema.omit({ status: true });
...
export function RestockOrderDraftForm({ initialData, suppliers, prefill = null }: RestockOrderDraftFormProps) {
  ...
  const supplierOptions = useMemo(
    () =>
      suppliers.map((supplier) => ({
        value: supplier.id,
        label: supplier.name,
        description: supplier.leadTimeDays ? `Entrega en ${supplier.leadTimeDays} días` : "Sin plazo registrado",
      })),
    [suppliers],
  );
  ...
  const onDelete = async () => {
    ...
    const confirmed = await requestConfirmation({
      title: `Eliminar el borrador ${displayRestockOrderNumber(initialData.orderNumber)}`,
  ...
  return (
    <>
      ...
      {leaveDialog}
      <FormPageHeader
        title={initialData ? `Pedido ${displayRestockOrderNumber(initialData.orderNumber)}` : "Nuevo pedido a proveedor"}
        badge={<TintBadge label={RESTOCK_STATUS_LABELS.DRAFT} tone="slate" />}
        summary={
          initialData
            ? "Sigue siendo un borrador: nada afecta al inventario hasta pedirlo y recibirlo."
            : "Registra lo que vas a pedir; el número se asigna al guardar."
        }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit((data) => save(data, RestockOrderStatus.DRAFT))} className="flex flex-col gap-4">
          ...
          <FormStickyFooter note="Guardar como borrador no afecta nada. «Pedir al proveedor» fija las líneas y habilita la recepción.">
            {canWrite && (
