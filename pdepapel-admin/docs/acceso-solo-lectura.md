...
**Pantallas reservadas**, escondidas del menú: Punto de venta, Clientes (la lista y el detalle), Mercado Libre, Conversaciones, Proveedores, Movimientos, Aprovisionamiento, Boletín, Rendimiento (sus dos gemelas `/negocio` e `/inteligencia-negocio` dejaron de ser pantallas el 2026-09-21: solo redirigen, no leen nada), Tributarios y Ajustes. Si alguien entra por la URL, `app/(dashboard)/[storeId]/error.tsx` explica que la pantalla es solo para la dueña, dentro del panel y sin parecer una falla.
...
> **Clientas con nombre de usuario (2026-09-21).** Meta omite `wa_id` de quien tiene nombre de usuario salvo que haya habido trato en 30 días o esté en la libreta del negocio —y contestarle **no** basta: se comprobó con una conversación de venta de veinte minutos en la que nunca apareció—. En su lugar manda un BSUID (`user_id`, `from_user_id`, `to_user_id`) y el nombre de usuario en `contacts[].profile.username`. Como todo se identificaba por teléfono, se perdieron en silencio **147 mensajes de 9 contactos** (70 de clientas y 77 respuestas de Paula), seis de ellas conversaciones de venta completas. `Conversation` gana `bsuid` y `username`, `phone` pasa a ser opcional, y la conversación se resuelve por BSUID con el teléfono como preferencia cuando llega. Para escribirles, Meta manda el BSUID en `recipient` y **sin** `to` (`lib/whatsapp/send.ts`).
